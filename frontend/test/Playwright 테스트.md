...
- 로그인 흐름에서는 어떻게 로그인하지?
    ...
    멀티탭 / 멀티 브라우저가 나오면서 궁금해진 건데
    생각해보니까 “유저, 어드민 화면을 보려면 로그인을 해야하고 그러려면 아이디, 비밀번호가 필요한데 이걸 그냥 올리면 테스트 계정이 털리는데” 하고 알아보니까
    테스트 계정까지 env에 빼둔다는 걸 알았다. 이렇게 보니 정말 E2E 테스트가 중요하다는 게 왜인지 모르게 다시 한 번 느껴진다.
